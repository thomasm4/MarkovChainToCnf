...
# Variation utilizing implications of states and it's transitions to avoid using state exclusion
    ...
    for state in states:
        ...
        for s in range(steps):
            ...
            for tra in filtered:
                ...
                clause = Or(endState, Neg(tAtom))
                clause.clausify()
                clauses.append(clause.clauses[0])

                atoms.append(tAtom)
            clause = Or(Neg(endState), *atoms)
            clause.clausify()
            clauses.append(clause.clauses[0])
    return clauses
...
def stateImpliesTrans(trans: list[Transition], steps: int = 1):
    clauses = []
    for s in range(steps):
        for tra in trans:
